...
* Nicht alle Wissenschaftler:innen wollen ihre Daten systematisch aufbereiten und der Öffentlichkeit zur Verfügung stelle..
...
\item Repräsentation zeilenweise miteinander verknüpfter Datensätze
  ...
  \end{itemize}
\item Erste Zeile (Kopfdatensatz/Header) ist enthält typischerweise die Definition der Spaltennamen
...
skriptordnder_pfad  <- dirname(r_skript_pfad)    # Ordner, in dem sich das Skript befindet
print(skriptordnder_pfad)
```

\tiny
```{r, eval = T, echo = F}
r_skript_pfad <- normalizePath(knitr::current_input())
r_skriptordnder_pfad  <- dirname(r_skript_pfad)
print(r_skriptordnder_pfad)
...
\setstretch{1.2}
```{r, eval = T}
#| label: Load data (read.table)
work_dir_path <- getwd()                                     # Pfad zum Working directory
data_dir_path <- file.path(dirname(work_dir_path), "Daten")  # Datenverzeichnispfad
file_name     <- "cushny.csv"                                # (base) filename
file_path     <- file.path(data_dir_path, file_name)         # filepath 
D             <- read.csv(file_path, header = TRUE, sep = ",")  # Einlesen der Datei
print(D)
```
...
Einige weitere Spezifikationen bei Anwendung von `read.csv()` sind
...
```{r, eval = T}
D <- read.csv(file_path, header = TRUE, sep = ",", nrow = 2)  # Auswahl von nur 2 Zeilen
print(D)
D <- read.csv(file_path, header = TRUE, sep = ",", skip = 7)  # Auswahl ab Zeile 7
print(D)
```
