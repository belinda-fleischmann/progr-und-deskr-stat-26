...
\textbf{1}  & \textbf{Mi, 15.10.} & \textbf{Do, 16.10.}  & \textbf{Seminar} & \textbf{(1) Grundbegriffe der Informatik}                        \\
...
3. VSCode für R startklar machen ([\textcolor{linkblue}{R in Visual Studio Code}](https://code.visualstudio.com/docs/languages/r)).
4. Mit dem eigenen oder Labor-Rechner vertraut machen. \small \justifying
    a) \small Wie viel Festplatten- und Arbeitsspeicher (RAM) besitzt der Rechner? Notieren Sie die Werte in GB.
    b) Welches Betriebssystem in welcher Version ist installiert? (z.B. Windows 11, macOS 14.5, Ubuntu 22.04)
    c) Erstellen Sie einen Ordner für alle Materialien und Übungen dieses Kurses (z.B. "PDS\_2026") an einem selbst gewählten Speicherort auf Ihrer Festplatte (nicht auf dem Desktop oder in Downloads).
    d) Wie lautet der vollständige absolute Pfad zu diesem Kursordner? (z.B. "C:\\Users\\IhrName\\Dokumente\\PDS_2026" oder "/home/username/Documents/PDS\_2026")
...
5. Erkunden Sie die VSCode User Interface und bearbeiten Sie nachstehenden Fragen (Tipp: Konsultieren Sie dafür die Online-Dokumentation [\textcolor{linkblue}{VSC User interface}](https://code.visualstudio.com/docs/getstarted/userinterface)): \small \justifying
    a) \small Welche verschiedenen Views (Ansichten) können in der Activity Bar (vertikale Leiste ganz links) ausgewählt werden? Listen Sie mindestens fünf auf.
    b)  Wählen Sie in der \textit{Activity Bar} den \textit{Explorer} aus. Wie können Sie Ihren Kursordner in VSCode öffnen, sodass dessen Inhalt in der Primary Sidebar (Hauptseitenleiste) dauerhaft sichtbar ist?
    c) Installieren Sie die R Extension für VSCode, falls noch nicht geschehen. Wählen Sie in der \textit{Activity Bar} die \textit{R Extension} aus. Welche Bereiche und Funktionen werden nun in der Primary Sidebar angezeigt?
    d) Standardmäßig befindet sich das Panel (mit Terminal, R-Console, etc.) am unteren Bildschirmrand. Wie können Sie es so konfigurieren, dass das Panel stattdessen rechts (oder links) vertikal angeordnet ist?
    e) Wie können Sie eine Secondary Sidebar öffnen und wofür könnte diese nützlich sein?
