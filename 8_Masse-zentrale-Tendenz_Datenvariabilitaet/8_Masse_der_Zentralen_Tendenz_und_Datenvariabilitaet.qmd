...
Dann ist der \textit{Modalwert (oder Modus)} von $y$ definiert als
\begin{equation}
...
\vspace{2mm}
Bestimmung der empirischen Varianz anhand der in \eqref{eq:varianz_unkorr.} und \eqref{eq:varianz} festgelegten Formeln
\vspace{2mm}
...
\footnotesize
\begin{theorem}[Empirische Varianz bei linear-affinen Transformationen]
\justifying
\normalfont
...
# Empirische Varianz bei linear-affinen Transformationen
\textcolor{darkcyan}{Beispiel: Empirische Varianz bei linear-affinen Transformationen}
...
s := \sqrt{s^2}
\end{equation}
und die \textit{unkorrigierte empirische Standardabweichung} von $y$ ist 
definiert als 
...
$z = (ay_1+b, ..., ay_n+b)$ sei der mit $a,b \in \mathbb{R}$ linear-affin
...
\setstretch{1}
```{r, eval = T}
#| label: lin.-affine Transformation Standardabweichung
# a >= 0
y   <- D$Pre.BDI                                                       # Pre-Interventions-BDI-II-Daten
a   <- 2                                                               # Multiplikationskonstante
b   <- 5                                                               # Additionskonstante
