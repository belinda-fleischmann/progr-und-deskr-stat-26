...
\textbf{2}  & \textbf{Mi, 22.10.} & \textbf{Do, 23.10.}  & \textbf{Seminar} & \textbf{(2) Arithmetik und Variablen}                         \\
...
# (2) Arithmetik und Variablen - Titelfolie {.plain}
\vfill
\center
\huge
\textcolor{black}{(2) Arithmetik und Variablen}
\vfill
...
# Arithmetik und Variablen ====================================== {.plain}
\AtBeginSection{}
...
![](../VL_Abbildungen/pds_2_VSCode_R_Umgebung.png){width="80%" fig-align="center"}
...
![](../VL_Abbildungen/pds_2_R_Console_oeffnen.png){width="80%" fig-align="center"}
...
* Code Ausführungsstopp mit \fbox{Ctrl} + \fbox{C}

\vspace{2mm}
\normalsize
\textcolor{darkcyan}{Beispiele für Befehle}
...
\begin{itemize} \footnotesize \justifying
  \item Der einfachste und zuverlässigste Weg, Hilfe zu Funktionen zu bekommen, ist über die in der lokalen Installation integrierten Dokumentation (Help pages).
...
\begin{itemize} \footnotesize \justifying
  ...
  \item  Wie wir später noch sehen werden, gibt es weitere Ausdrücke in R, die keine Funktionen sind. In den meisten solcher Fälle finden wir die entsprechende Help page, indem der Ausdruck in Anführungszeichen gesetzt wird.
\end{itemize}
...
![](../VL_Abbildungen/pds_2_R_helpviewer.png){width="70%" fig-align="center"}
...
# R Skripte $\vert$ Editing and Executing Code
\vspace{2mm}
\setstretch{1.7}
...
\textcolor{darkblue}{Tipps für ein strukturiertes und übersichtliches R Skript}

\small
\color{black}

\begin{itemize}
\item Beginnen Sie ein R Skript mit einem \textbf{Header}, welche die wichtigsten Informationen zu dem vorliegenden Programm enthält.
\item Achten Sie darauf, möglichst jeden Befehl mit einem \textbf{Kommentar} zu versehen, sodass leicht nachvollzogen werden kann, was in jeder Zeile passiert.
\end{itemize}
...
\item Ein R Paket ist eine Sammlung von R-Funktionen, Datensätzen und vordefinierten Skripten, die in einem strukturierten Format (Paket) gebündelt sind.
\item Analog zu Computerprogrammen (wie MS Word) müssen R Pakete zuerst auf dem Rechner \textbf{installiert} werden, bevor sie verwendet werden können. Ausnahmen sind Standardpakete (\texttt{standard library}), die automatisch mit der Installation von R mitgeliefert werden.
\item Wie Computerprogramme müssen R Pakete auch \underline{nur \textit{einmal}} installiert werden. Deshalb empfiehlt es sich, Paketinstallationsbefehle aus Skripten auszukommentieren.
\item Analog dazu, dass Computerprogramme bei jedem Neustart zuerst \textit{geöffnet} werden müssen, bevor damit gearbeitet werden kann, müssen R Pakete bei jeder neuen R Sitzung zuerst in die Workspace \textbf{geladen} werden, bevor damit gearbeitet werden kann.
\end{itemize}
...
\begin{itemize} \small
  \item Anzeigen aller installierter Pakete in der R Extension: \textit{HELP PAGES} $\to$ \textit{Home} $\to$ \textit{Packages} oder mit dem R Befehl \texttt{installed.packages()}.
  \item Das \textbf{Installieren} eines Pakets erfolgt mit dem R Befehl \texttt{install.packages("\text{paketname}")}. Da das Paket vor Installation noch nicht auf dem System existiert, wird der Paketname als string, d.h. \textbf{mit Anführungsstrichen} an die Funktion übergeben.
  \item Das \textbf{Laden} eines Pakets erfolgt mit dem R Befehl \texttt{library(paketname)}. Da das Paket nach Installation auf dem System existiert und referenziert werden kann, wird der Paketname als Variable, d.h. \textbf{ohne Anführungsstriche} an die Funktion übergeben.
  \item Tipp: Integrieren Sie das Paket-Management in Ihr R Skript. 
  \begin{itemize} \small
    \item Wenn in einem Datenanalyseskript Pakete außerhalb der `standard library` verwendet werden, sollte das Laden des Pakets auch ein Befehl im Analyseskript sein.
    \item Das gilt \textbf{nicht} für Installationsbefehle (\texttt{install.packages()})! Die Installation von Paketen ist \textbf{nur einmal} notwendig. Falls Installationsbefehle im Skript stehen, sollten sie auskommentiert werden, da sie sonst unnötig Zeit und Rechenressourcen kosten.
    \item Laden Sie alle benötigten Pakete zu Beginn des Skripts und halten Sie in einem Kommentar fest, wofür es benötigt wird.
  \end{itemize}
\end{itemize}
...
\textcolor{darkblue}{Beispiel eines strukturierten R Skripts, das zusätzliche Pakete verwendet}
...
library(lobstr)                     # Laden des Pakets lobstr; für obj_addr()

# --- Variablendefinition ---
Ausgabetext <- "Hallo Welt!"        # Definition der Variable 'Ausgabetext'

# --- Ausgabe ---------------
print(Ausgabetext)                  # Ausgabe der Variabe 'Ausgabetext'
obj_addr(Ausgabetext)               # Ausgabe der Speicheradresse der Variable 'Ausgabetext'
```
...
![](../VL_Abbildungen/pds_2_precedence.pdf){width="68%"}
...
\vfill
![](../VL_Abbildungen/pds_2_vscode_workspace.pdf){width="60%"}
...
![](../VL_Abbildungen/pds_2_binding_1.pdf){width="22%"}
...
![](../VL_Abbildungen/pds_2_binding_2_WiSe23.pdf){width="44%"}
...
![](../VL_Abbildungen/pds_2_copy_on_modify.pdf){width="70%"}


* R Objekte sind \textit{immutable}, können also nicht verändert werden.
* Bei Modifizierung der Variable \texttt{y} wird das \textquote{Original}-Objekt (0x74b) nicht verändert, sondern eine Kopie (0xcd2) erstellt, welche dann verändert wird.
...
2. \small Environments werden in-place modifiziert.
...
![](../VL_Abbildungen/pds_2_copy_on_modify_2.pdf){width="70%"}
...
![](../VL_Abbildungen/pds_2_unbinding.pdf){width="70%"}
...
![](../VL_Abbildungen/pds_2_r_objektstruktur.pdf){width="80$%}
...
```
```{r, eval = T}
#| label: test type numeric
is.numeric(x)
```
```{r, eval = T}
#| label: test type double on int
is.double(y)
```
```{r, eval = T}
#| label: test type numeric on int
is.numeric(y)
```
...
$\Rightarrow$ (3) Vektoren und Matrizen
