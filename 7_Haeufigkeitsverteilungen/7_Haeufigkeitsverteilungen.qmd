...
* Deskriptivstatistiken bieten graphische Darstellungen und zusammenfassende Maßzahlen, die charakteristische Aspekte eines Datensatzes wie seine "durchschnittlicher" Wert oder seine Variabilität beschreiben.
...
* Auch wenn diese Einheit ohne wahrscheinlichkeitstheoretischen Hintergrund auskommt, erschließt sich seine volle Bedeutung erst im Kontext der auskommt.
...
names(H) <- c("w", "ah")               # Benennen der Spalten im Dataframe
```
...
\footnotesize
```{r, eval = T}
H$rh     <- H$ah / n                   # Neue Spalte mit relativen Häufigkeiten
...
```{r, eval = F}
h          <- H$ah            # Kopie der absoluten Häufigkeiten in einen Vektor
...
$h := 3.49 \cdot \frac{S}{\sqrt[3]{n}}$ & Berechnung $h$ nach Scott (1979): Min. MSE Dichteschätzung bei Normalverteilung
\end{tabular}
\end{center}
...
# Histogramme - \textcolor{darkcyan}{Beispiel}
nach @scott_1979 $, h := 3.49S/\sqrt[3]{n}$
\vspace{1mm}
...
\end{equation}
die \textit{kumulative absolute Häufigkeitsverteilung} von $y$ und die Funktion
\begin{equation}
R : W \to [0,1], w \mapsto R(w) : =\sum_{w' \le w} r(w')
\end{equation}
die \textit{kumulative relative Häufigkeitsverteilung} der Zahlwerte von $y$.
\end{definition}
